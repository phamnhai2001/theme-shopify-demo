$(function(){
	...
	if(_DefMaxSlide =="") _DefMaxSlide ="60.0";
	...
	if(_DefPriceMaxSlide =="") _DefPriceMaxSlide ="600000";

	var priceValue=[0,100,500,1000,1500,2000,2500,3000,3500,4000,5000,6000,7000,8000,9000,10000,15000,20000,25000,30000,35000,40000,45000,50000,60000,70000,80000,90000,100000,150000,200000,250000,300000,350000,400000,450000,500000,550000,600000];
	var minPriceStep = 0;
	var maxPriceStep = 0;
	...
	$('#searchPrice').slider({
		...
		slide: function( event, ui ) {
			if( ui.handleIndex == "0") $('#p_pris').val(priceValue[ui.values[0]]);
			if( ui.handleIndex == "1") $('#p_prie').val(priceValue[ui.values[1]]);
		}
	});
